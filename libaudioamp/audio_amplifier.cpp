...
int tfa9887_set_mode(audio_mode_t mode) {
    struct tfa9887_reg_data amp_data[TFA9887_MAX_REG_DATA];
    ...
    switch(mode)
    {
        case AUDIO_MODE_NORMAL:
            ...
            length = sizeof(tfa9887_playback_data) / sizeof(tfa9887_reg_data);
            memcpy(&amp_data, tfa9887_playback_data, sizeof(tfa9887_playback_data));
            break;
        case AUDIO_MODE_RINGTONE:
            ALOGI("[TFA9887] Mode: Ring");
            length = sizeof(tfa9887_ring_data) / sizeof(tfa9887_reg_data);
            memcpy(&amp_data, tfa9887_ring_data, sizeof(tfa9887_ring_data));
            break;
        case AUDIO_MODE_IN_CALL:
        case AUDIO_MODE_IN_COMMUNICATION:
            ALOGI("[TFA9887] Mode: Voice");
            length = sizeof(tfa9887_voice_data) / sizeof(tfa9887_reg_data);
            memcpy(&amp_data, tfa9887_voice_data, sizeof(tfa9887_voice_data));
            break;
        default:
    ...
    for(i=0; i < length; i++)
	{
        reg_value[0] = sizeof(amp_data[i].val);
        reg_value[1] = amp_data[i].val;

        /* Write register values to the right amplifier */
        if((ret = ioctl(tfa9887_fd, TPA9887_WRITE_CONFIG, &reg_value)) != 0) {
            ALOGE("[TFA9887] ioctl %d failed. ret = %d", TPA9887_WRITE_CONFIG, ret);
        }

        /* Write register values to the left amplifier */
