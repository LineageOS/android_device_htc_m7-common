...
static int map_sysfs_string(const char* str) {
    ...
    for (i = 0; battery_status_map[i].s; i++) {
        if (!strcmp(str, battery_status_map[i].s))
...
static int get_charging_status() {
    ...
    if (bstat) {
        if (fgets(batt_stat_str, STR_BUF_SIZE, bstat) == NULL) {
            KLOG_ERROR(LOG_TAG, "%s: failed to read %s; errno=%d\n",
                __func__, BATTERY_STATUS_FILE, errno);
            return -(errno);
        }
        ...
        KLOG_ERROR(LOG_TAG, "%s: could not open %s; errno=%d\n",
            __func__, BATTERY_STATUS_FILE, errno);
        return -(errno);
    }
...
static int uevent_init() {
    ...
    if (uevent_fd >= 0) {
        ...
        KLOG_ERROR(LOG_TAG, "%s: uevent_open_socket failed; errno=%d\n",
            __func__, errno);
        return errno;
    }
...
static void chargeled_mainloop() {
    ...
    if (epollfd == -1) {
        KLOG_ERROR(LOG_TAG, "%s: epoll_create failed; errno=%d\n",
            __func__, errno);
        return;
    }
    ...
    if (epoll_ctl(epollfd, EPOLL_CTL_ADD, uevent_fd, &ev) == -1) {
        KLOG_ERROR(LOG_TAG,
            "%s: epoll_ctl for uevent_fd failed; errno=%d\n",
            __func__, errno);
        return;
    } else {
        maxevents++;
